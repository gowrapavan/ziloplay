...
import { useStorage } from '../hooks/useStorage'; // ⬅️ 1. Import hook
...
export default function Hero({ items = [] }) {
  ...
  // ⬅️ 2. Initialize Watchlist logic
  ...
  // Helper: Check if current item is in watchlist
  ...
  return (
    <div className="relative w-full h-[70vh] md:h-[95vh] overflow-hidden bg-[#121212] text-white font-sans group">
      ...
      <div className="absolute z-20 inset-0 flex flex-col justify-end md:justify-center pb-16 md:pb-0 px-6 md:px-16 w-full md:w-[60%]">
        <div className="flex flex-col items-center md:items-start space-y-4 animate-fadeIn text-center md:text-left">
          ...
          <div className="flex items-center gap-3 pt-2 w-full md:w-auto justify-center md:justify-start">
            ...
            {/* ⬅️ 3. Updated Watchlist Toggle Button */}
            <button 
              onClick={() => toggleWatchlist({ ...currentItem, mediaType: currentItem.media_type || 'movie' })}
              className={`w-10 h-10 md:w-11 md:h-11 flex items-center justify-center backdrop-blur-sm rounded-lg border transition-all duration-300 ${
                isSaved 
      ...
      {/* THUMBNAIL & BATCH NAVIGATION (Desktop) */}
      <div className="absolute z-30 bottom-4 right-4 hidden md:flex items-end gap-2 pl-10 bg-gradient-to-l from-black/90 to-transparent p-3 rounded-l-xl">
        {batchIndex > 0 && (
          <button onClick={handlePrevBatch} className="w-8 h-14 mb-[1px] flex items-center justify-center bg-white/10 hover:bg-white/30 rounded transition">
        ...
        {heroItems.map((item, index) => (
          <div
            ...
            className={`relative cursor-pointer transition-all duration-300 ease-out rounded overflow-hidden ${
              index === currentIndex ? 'w-32 h-16 border-2 border-white opacity-100 scale-105' : 'w-24 h-14 border border-transparent opacity-50 hover:opacity-100'
            }`}
          >
        ...
        {(batchIndex + 1) * ITEMS_PER_BATCH < items.length && (
          <button onClick={handleNextBatch} className="w-8 h-14 mb-[1px] flex items-center justify-center bg-white/10 hover:bg-white/30 rounded transition">
             <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
