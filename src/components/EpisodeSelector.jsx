...
export default function EpisodeSelector({ seriesId, seasons, onSelect, activeSeason, activeEpisode }) {
  ...
  return (
    <div 
      ref={containerRef} 
      className="bg-[#161616]/40 p-4 md:p-6 rounded-md border border-white/5 backdrop-blur-md mb-6 relative z-[100]"
    >
      {/* Header with Switcher Icons */}
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-3">
          <div className="h-5 w-1 bg-red-600 rounded-sm" />
          <h3 className="text-[11px] md:text-xs font-black uppercase tracking-[0.2em] text-gray-400">Select Episode</h3>
        </div>
        <div className="flex bg-[#0a0a0a] p-1 rounded-md border border-white/5">
          <button onClick={() => setViewMode("list")} className={`p-1.5 md:p-2 rounded-sm transition-all ${viewMode === "list" ? "bg-red-600 text-white" : "text-gray-500 hover:text-gray-300"}`}>
          ...
          <button onClick={() => setViewMode("grid")} className={`p-1.5 md:p-2 rounded-sm transition-all ${viewMode === "grid" ? "bg-red-600 text-white" : "text-gray-500 hover:text-gray-300"}`}>
      ...
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <div className="flex-1 relative">
          ...
          <div 
            ...
            className="bg-[#0a0a0a] text-white rounded-md py-2.5 px-4 w-full border border-white/10 flex justify-between items-center cursor-pointer font-bold text-xs"
          >
          ...
          {isSeasonOpen && (
            <div className="absolute top-[105%] left-0 w-full bg-[#0a0a0a] border border-white/10 rounded-md overflow-hidden shadow-[0_10px_30px_rgba(0,0,0,0.5)] max-h-[200px] overflow-y-auto scrollbar-hide z-[120]">
        ...
        {viewMode === "list" && (
          <div className="flex-[3] relative">
            ...
            {isEpisodeOpen && (
              <div className="absolute top-[105%] left-0 w-full bg-[#0a0a0a] border border-white/10 rounded-md overflow-hidden shadow-[0_10px_30px_rgba(0,0,0,0.5)] max-h-[250px] overflow-y-auto scrollbar-hide z-[120]">
                {episodes.map((ep) => (
                  <div
                    key={ep.id}
      ...
      {viewMode === "grid" && (
        <div className="flex overflow-x-auto gap-2 mt-4 scrollbar-hide">
          {episodes.map(ep => (
            <button key={ep.id} onClick={() => onSelect(activeSeason, ep.episode_number)} className={`flex-shrink-0 w-10 h-10 rounded-md border text-xs font-black transition-all ${activeEpisode === ep.episode_number ? 'bg-red-600 border-red-500 text-white shadow-[0_0_15px_rgba(229,9,20,0.3)]' : 'bg-[#0a0a0a] border-white/5 text-gray-400 hover:border-red-600/50'}`}>
              {ep.episode_number}
            </button>
          ))}
        </div>
